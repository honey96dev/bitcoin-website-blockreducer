...
const apiDataProc = (req, res, next) => {
    let sql = sprintf("SELECT I.*, I.option_symbol `aka` FROM `deribit_instruments2` I ORDER BY I.creation_timestamp;");
    dbConn.query(sql, undefined, (error, results, fields) => {
        if (error) {
            res.status(200).send([]);
            return;
...
router.get('/api/data', apiDataProc);
