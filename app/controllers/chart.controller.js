...
function GetVolumeChart(req, res) {
    ...
    sql = sprintf("SELECT * FROM (SELECT V.timestamp, IFNULL(V.volume, 0) volume, IFNULL(I.open, 0) `open`, IFNULL(W.vwap_seed, 1) `vwap_seed`, IFNULL(I.num_3, 0) `num_3`, IFNULL(I.num_6, 0) `num_6`, IFNULL(I.num_9, 0) `num_9`, IFNULL(I.num_100, 0) `num_100`  FROM `volume_%s` V LEFT JOIN `vwap_%s` W ON W.timestamp = V.timestamp LEFT JOIN `id0_%s` I ON I.timestamp = V.timestamp ORDER BY V.timestamp DESC LIMIT 2000) `tmp`  ORDER BY `timestamp` ASC;", interval, interval, interval);
